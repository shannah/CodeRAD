            ...
            for (Iterator e = ((List) node).iterator(); e.hasNext();) {
                ...
                if ((tmp instanceof Map)) {
                    ...
                    if (tmpMap.containsKey(name)) {
                        ...
                        Object keyVal = tmpMap.get(name);
                        if (keyVal instanceof List) {
                            List arr = _asStructuredContentArray((List) keyVal);
                            if (arr != null) {
                                out.addAll(arr);
                            }
                        } else if (keyVal instanceof Map) {
                            out.add(new MapContent((Map)keyVal, this));
                        } else if (keyVal != null) {
                            out.add(new MapContent(keyVal.toString(), this));
                        }
                    }
                }
                tmp = null;
